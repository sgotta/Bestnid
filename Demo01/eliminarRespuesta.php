<?php
	...
	OR (getdate()['year'] == date("Y",$fecha_fin_subasta) AND getdate()['mon'] == date("n",$fecha_fin_subasta) AND getdate()['mday'] <= date("j",$fecha_fin_subasta))){
		$idNotificacion = $comentario['notificacion_respuesta'];
		...
		mysql_query("UPDATE comentario SET respuesta = NULL WHERE idComentario = '$comentario[idComentario]'") or die ("problemas en consulta:".mysql_error());
		echo "Eliminada";
	}
	else {
		echo "Fecha";
